...
export async function generateRouter(routes, buildDir, logger) {
  ...
  // ✅ FIX: Use React.createElement with proper syntax
...
export function Router({ routes }) {
  ...
  return React.createElement(RouterContext.Provider, { value: routerValue }, Component ? React.createElement(Component, { params }) : React.createElement(NotFound, null));
}
...
function NotFound() {
  ...
  const h1Style = { fontSize: '6rem', margin: 0 };
  const pStyle = { fontSize: '1.5rem', color: '#666' };
  const aStyle = { color: '#10b981', textDecoration: 'none', fontSize: '1.2rem' };
  
  return React.createElement('div', { style: containerStyle }, 
    React.createElement('h1', { style: h1Style }, '404'),
    React.createElement('p', { style: pStyle }, 'Page not found'),
    React.createElement('a', { href: '/', style: aStyle }, 'Go home')
  );
}
