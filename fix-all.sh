...
cd /home/PeaseErnest/Desktop/Node\ libs/ernest

echo "🔧 Fixing JavaScript syntax error in bundler.js..."

# Create a clean version of the bundleJavaScript function
cat > /tmp/fixed_bundle.js << 'EOF'
...
# Now replace the function in bundler.js
# First, find where the function starts and ends
echo "🔍 Finding bundleJavaScript function in bundler.js..."

# Backup original
cp src/ui/bundler.js src/ui/bundler.js.backup

# Use sed to replace the function
# We'll replace from "async function bundleJavaScript" to the closing brace
sed -i '/async function bundleJavaScript/,/^}$/ {
  /async function bundleJavaScript/ {
    r /tmp/fixed_bundle.js
    d
  }
  /async function bundleJavaScript/,/^}$/d
}' src/ui/bundler.js

# Check if replacement worked
if grep -q "async function bundleJavaScript" src/ui/bundler.js; then
  echo "✅ Function replaced successfully"
else
  echo "⚠️  Function not found, trying alternative approach..."
  
  # Alternative: just replace the entire file with a clean version
  cat > src/ui/bundler.js.clean << 'EOF'
...
export async function buildUI(config, logger) {
  ...
  try {
    ...
    // Step 3: Generate router (if we have routes)
    if (routes.length > 0) {
...
EOF
  ...
  mv src/ui/bundler.js.clean src/ui/bundler.js
fi

echo "✅ Fix applied!"
echo "🚀 Now test: ernest build --mode ui"